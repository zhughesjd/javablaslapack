...
*<br>
*> \htmlonly<br>
*> Download SBBCSD + dependencies <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sbbcsd.f"> <br>
*> [TGZ]</a> <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sbbcsd.f"> <br>
*> [ZIP]</a> <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sbbcsd.f"> <br>
*> [TXT]</a><br>
*> \endhtmlonly <br>
*<br>
*  Definition:<br>
*  ===========<br>
*<br>
*       SUBROUTINE SBBCSD( JOBU1, JOBU2, JOBV1T, JOBV2T, TRANS, M, P, Q,<br>
...
*> \verbatim<br>
*><br>
*> SBBCSD computes the CS decomposition of an orthogonal matrix in<br>
...
*> the TRANS and SIGNS options. See SORCSD for details.)<br>
*><br>
...
*> \param[in,out] U1<br>
*> \verbatim<br>
*>          U1 is REAL array, dimension (LDU1,P)<br>
...
*> \param[in,out] U2<br>
*> \verbatim<br>
*>          U2 is REAL array, dimension (LDU2,M-P)<br>
...
*> \param[in,out] V1T<br>
*> \verbatim<br>
*>          V1T is REAL array, dimension (LDV1T,Q)<br>
...
*> \param[in,out] V2T<br>
*> \verbatim<br>
*>          V2T is REAL array, dimenison (LDV2T,M-Q)<br>
...
*> \param[out] B11D<br>
*> \verbatim<br>
*>          B11D is REAL array, dimension (Q)<br>
*>          When SBBCSD converges, B11D contains the cosines of THETA(1),<br>
*>          ..., THETA(Q). If SBBCSD fails to converge, then B11D<br>
...
*> \param[out] B11E<br>
*> \verbatim<br>
*>          B11E is REAL array, dimension (Q-1)<br>
*>          When SBBCSD converges, B11E contains zeros. If SBBCSD fails<br>
...
*> \param[out] B12D<br>
*> \verbatim<br>
*>          B12D is REAL array, dimension (Q)<br>
*>          When SBBCSD converges, B12D contains the negative sines of<br>
*>          THETA(1), ..., THETA(Q). If SBBCSD fails to converge, then<br>
...
*> \param[out] B12E<br>
*> \verbatim<br>
*>          B12E is REAL array, dimension (Q-1)<br>
*>          When SBBCSD converges, B12E contains zeros. If SBBCSD fails<br>
...
*> \param[out] B21D<br>
*> \verbatim<br>
*>          B21D is REAL array, dimension (Q)<br>
*>          When SBBCSD converges, B21D contains the negative sines of<br>
*>          THETA(1), ..., THETA(Q). If SBBCSD fails to converge, then<br>
...
*> \param[out] B21E<br>
*> \verbatim<br>
*>          B21E is REAL array, dimension (Q-1)<br>
*>          When SBBCSD converges, B21E contains zeros. If SBBCSD fails<br>
...
*> \param[out] B22D<br>
*> \verbatim<br>
*>          B22D is REAL array, dimension (Q)<br>
*>          When SBBCSD converges, B22D contains the negative sines of<br>
*>          THETA(1), ..., THETA(Q). If SBBCSD fails to converge, then<br>
...
*> \param[out] B22E<br>
*> \verbatim<br>
*>          B22E is REAL array, dimension (Q-1)<br>
*>          When SBBCSD converges, B22E contains zeros. If SBBCSD fails<br>
...
*>          > 0:  if SBBCSD did not converge, INFO specifies the number<br>
...
*><br>
*> \verbatim<br>
*>  TOLMUL  REAL, default = MAX(10,MIN(100,EPS**(-1/8)))<br>
...
*<br>
*> \htmlonly<br>
*> Download SBDSDC + dependencies <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sbdsdc.f"> <br>
*> [TGZ]</a> <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sbdsdc.f"> <br>
*> [ZIP]</a> <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sbdsdc.f"> <br>
*> [TXT]</a><br>
*> \endhtmlonly <br>
*<br>
*  Definition:<br>
*  ===========<br>
*<br>
*       SUBROUTINE SBDSDC( UPLO, COMPQ, N, D, E, U, LDU, VT, LDVT, Q, IQ,<br>
*                          WORK, IWORK, INFO )<br>
* <br>
*       .. Scalar Arguments ..<br>
...
*> \verbatim<br>
*><br>
*> SBDSDC computes the singular value decomposition (SVD) of a real<br>
...
*> respectively. SBDSDC can be used to compute all singular values,<br>
...
*> without guard digits, but we know of none.  See SLASD3 for details.<br>
*><br>
*> The code currently calls SLASDQ if singular values only are desired.<br>
...
*> \param[out] Q<br>
*> \verbatim<br>
*>          Q is REAL array, dimension (LDQ)<br>
*>          If  COMPQ = 'P', then:<br>
...
*>             In particular, Q contains all the REAL data in<br>
...
*<br>
*> \htmlonly<br>
*> Download SBDSQR + dependencies <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sbdsqr.f"> <br>
*> [TGZ]</a> <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sbdsqr.f"> <br>
*> [ZIP]</a> <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sbdsqr.f"> <br>
*> [TXT]</a><br>
*> \endhtmlonly <br>
*<br>
*  Definition:<br>
*  ===========<br>
*<br>
*       SUBROUTINE SBDSQR( UPLO, N, NCVT, NRU, NCC, D, E, VT, LDVT, U,<br>
...
*> \verbatim<br>
*><br>
*> SBDSQR computes the singular values and, optionally, the right and/or<br>
...
*> form:  A = U*B*VT, as computed by SGEBRD, then<br>
*> <br>
*>    A = (U*Q) * S * (P**T*VT)<br>
*> <br>
...
*<br>
*> \htmlonly<br>
*> Download SBDSVDX + dependencies <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/sbdsvdx.f"> <br>
*> [TGZ]</a> <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/sbdsvdx.f"> <br>
*> [ZIP]</a> <br>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/sbdsvdx.f"> <br>
*> [TXT]</a><br>
*> \endhtmlonly <br>
*<br>
*  Definition:<br>
*  ===========<br>
*<br>
*     SUBROUTINE SBDSVDX( UPLO, JOBZ, RANGE, N, D, E, VL, VU, IL, IU, <br>
...
*> \verbatim<br>
*><br>
*>  SBDSVDX computes the singular value decomposition (SVD) of a real<br>
*>  N-by-N (upper or lower) bidiagonal matrix B, B = U * S * VT, <br>
...
*>  and superdiagonal E = [ e_1 e_2 ... e_N-1 ], SBDSVDX computes the <br>
...
*>  descending order (as in SGESVD/SGESDD) or b) compute s,v and reorder <br>
*>  the values (and corresponding vectors). SBDSVDX implements a) by <br>
*>  calling SSTEVX (bisection plus inverse iteration, to be replaced <br>
...
*> \param[out] Z<br>
*> \verbatim<br>
*>          Z is REAL array, dimension (2*N,K) )<br>
...
*> \param[out] WORK<br>
*> \verbatim<br>
*>          WORK is REAL array, dimension (14*N)<br>
*> \endverbatim<br>
*><br>
*> \param[out] IWORK<br>
*> \verbatim<br>
...
*>                   in SSTEVX. The indices of the eigenvectors<br>
*>                   (as returned by SSTEVX) are stored in the<br>
*>                   array IWORK.<br>
